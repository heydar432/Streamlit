...
if st.session_state.question_number < len(st.session_state.random_indices):
    ...
    if st.session_state.incorrect_answers:
        ...
        for term, defs, pron in st.session_state.incorrect_answers:
            ...
            st.markdown(f"<h4 style='text-align: left; color: black;'>Definition: <span style='color: red;'>{defs}</span></h4>", unsafe_allow_html=True)
            st.markdown(f"<h4 style='text-align: left; color: black;'>Pronunciation: <span style='color: red;'>{pron}</span></h4>", unsafe_allow_html=True)
