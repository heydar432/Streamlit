...
if st.session_state.question_number < len(st.session_state.random_indices):
    ...
    if st.session_state.incorrect_answers:
        ...
        for term, defs, pron in st.session_state.incorrect_answers:
            ...
            st.markdown(f"<h3 style='text-align: left; color: yellow;'>Definition: {defs}</h3>", unsafe_allow_html=True)
            st.markdown(f"<h3 style='text-align: left; color: yellow;'>Pronunciation: {pron}</h3>", unsafe_allow_html=True)
