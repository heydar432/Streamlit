...
if st.session_state.question_number < len(st.session_state.random_indices):
    ...
    if st.session_state.incorrect_answers:
        ...
        for term, defs, pron, user_ans in st.session_state.incorrect_answers:
            ...
            st.markdown(f"<h4 style='text-align: left; color: black; font-size: 20px;'> 📝 <span style='color: red; font-style: italic;'>{defs}</span></h4>", unsafe_allow_html=True)
            st.markdown(f"<h4 style='text-align: left; color: black; font-size: 20px;'> 📣 <span style='color: red; font-style: italic;'> [ {pron} ]</span></h4>", unsafe_allow_html=True)
