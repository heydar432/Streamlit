...
# Function to create the finance URL
def create_finance_url(symbol):
    return f"https://www.google.com/finance/quote/{symbol}:NASDAQ?sa=X&ved=2ahUKEwjC-tW_qYuIAxUrGhAIHXg3Kj0Q3ecFegQIMxAf"

# Add a column for clickable links
df['Symbol'] = df['Symbol'].apply(lambda x: f'<a href="{create_finance_url(x)}" target="_blank">{x}</a>')
...
# Display the filtered data with a wide table and clickable links
